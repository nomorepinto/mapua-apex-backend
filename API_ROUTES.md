...
| **GET** | `/api/v1/admins/organizations` | **Response `200`**. `signatories` is the org desk list (`adviser`, `cdm`, `dean`). Empty until desks are assigned on the organization. <pre>{<br>  "data": [<br>    {<br>      "organization_id": "a1b2",<br>      "name": "Mapua Computing Society",<br>      "signatories": [<br>        { "role": "adviser", "signatory_id": "adv001" },<br>        { "role": "cdm", "signatory_id": "cdm001" },<br>        { "role": "dean", "signatory_id": "dean001" }<br>      ]<br>    }<br>  ]<br>}</pre> |
...
| **GET** | `/api/v1/admins/signatories` | **Response `200`**. `department` is set only for deans. <pre>{<br>  "data": [<br>    {<br>      "signatory_id": "adv001",<br>      "name": "Prof. Juan Dela Cruz",<br>      "role": "adviser",<br>      "department": null,<br>      "organization_id": null<br>    }<br>  ]<br>}</pre>`role` is `adviser`, `cdm`, or `dean`. |
| **POST** | `/api/v1/admins/signatories` | **Request body.** Creates the SIGNATORY item. GSI4 is `ROLE#{ROLE}`, or `ROLE#DEAN#{DEPARTMENT}` when `role` is `dean` and `department` is sent (e.g. `SOIT`). `department` is ignored for adviser/CDM. **Response `201`**. Put `signatory_id` on the Cognito user as `custom:signatory_id`. <pre>{<br>  "name": "Prof. Juan Dela Cruz",<br>  "role": "adviser"<br>}</pre>Dean example: <pre>{<br>  "name": "Dean Maria Santos",<br>  "role": "dean",<br>  "department": "SOIT"<br>}</pre> |
| **PUT** | `/api/v1/admins/signatories/{signatory}` | **Request body** — `name`, `role`, and optional `department` (dean only). The path `{signatory}` is the signatory id. Updates that SIGNATORY item and GSI4 (`ROLE#DEAN#SOIT` when department is set). Unknown id → **404**. Does **not** rewrite past `NOTIFICATION.signatory` snapshots. **Response `200`**. <pre>{<br>  "name": "Prof. Juan Dela Cruz",          ← CHANGE THIS if the person changed<br>  "role": "dean",                          ← CHANGE THIS (adviser \| cdm \| dean)<br>  "department": "SOIT"                     ← CHANGE THIS for deans only<br>}</pre> |
