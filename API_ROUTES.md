...
| **GET** | `/api/v1/admins/signatories` | **Response `200`**. <pre>{<br>  "data": [<br>    {<br>      "signatory_id": "adv001",<br>      "name": "Prof. Juan Dela Cruz",<br>      "role": "adviser",<br>      "organization_id": null<br>    }<br>  ]<br>}</pre>`role` is `adviser`, `cdm`, or `dean`. |
| **POST** | `/api/v1/admins/signatories` | **Request body.** Creates the SIGNATORY item (`name`, `role`, GSI4 `ROLE#{ROLE}`). Does **not** take `organization_id` and does **not** write `ORGANIZATION.signatories`. **Response `201`**. Put `signatory_id` on the Cognito user as `custom:signatory_id`. <pre>{<br>  "name": "Prof. Juan Dela Cruz",<br>  "role": "adviser"<br>}</pre> |
| **PUT** | `/api/v1/admins/signatories/{signatory}` | **Request body** — `name` and `role` only. The path `{signatory}` is the signatory id (`adv001` or `SIGNATORY#adv001`). Updates that SIGNATORY item and GSI4 `ROLE#{ROLE}`. Does **not** take `organization_id` and does **not** write `ORGANIZATION.signatories`. Unknown id → **404**. Does **not** rewrite past `NOTIFICATION.signatory` snapshots. **Response `200`**. <pre>{<br>  "name": "Prof. Juan Dela Cruz",          ← CHANGE THIS if the person changed<br>  "role": "dean"                           ← CHANGE THIS (adviser \| cdm \| dean)<br>}</pre> |
